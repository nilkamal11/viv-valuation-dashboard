...
const sc=read('standard-chartered'),scGuide=read('standard-chartered-fund-guide'),tcg=read('tcg-qcam');
...
assert.ok(homePage.includes('value="standard-chartered"')&&homePage.includes("{tab==='standard-chartered'?"));assert.ok(scPage.includes('not added to the LGT totals')&&!/(glare|obscured|mobile|photograph|Source clarity|Not legible|FINDING THIS VIEW ONLINE)/i.test(scPage));assert.ok(!/retail\.sc\.com|IBAN|OTP|\br=/.test(JSON.stringify(sc)+scPage));
assert.equal(cents(tcg.net_capital_invested+tcg.lifetime_profit),cents(tcg.current_portfolio_value));assert.equal(cents(tcg.transactions.reduce((a,r)=>a+r.amount,0)),cents(tcg.net_capital_invested));assert.equal(tcg.monthly_history.length,12);assert.equal(sum(Object.values(tcg.allocation_current_pct).map(cents)),10000);assert.equal(sum(Object.values(tcg.allocation_lifetime_pct).map(cents)),10000);assert.ok(homePage.includes('value="tcg-qcam"')&&homePage.includes('<TcgQcam/>'));
const tcgPage=fs.readFileSync(new URL('../app/tcg-qcam.tsx',import.meta.url),'utf8');assert.ok(tcgPage.includes('HIGH VERIFICATION RISK')&&tcgPage.includes('not proven'));assert.ok(!/GFS0000121FI00|720830173|vipinshah|Lower Kabete|Vipinchandra/.test(tcgPage+JSON.stringify(tcg)));
assert.equal(cents(p.summary.net),830340626);assert.equal(h.holdings.length,336);
console.log('PASS: TCG statement arithmetic, contribution history, allocation totals, public-risk review and PII exclusions.');
